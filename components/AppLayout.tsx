...
export function AppLayout({ children, activeView, onNavigate, onDeposit, onWithdraw, user, onLogout }: AppLayoutProps) {
  ...
  const slipCount = useBetStore((state) => state.betSlip.length);
  useEffect(() => {
    if (!mobileSports && !mobileSlip) return;
    const previous = document.body.style.overflow;
    document.body.style.overflow = "hidden";
    return () => { document.body.style.overflow = previous; };
  }, [mobileSlip, mobileSports]);
  return (
    <div className="app-shell">
      ...
      <nav className="mobile-nav">
        ...
        <button className="bet-slip-fab" onClick={() => setMobileSlip(true)} aria-label={`Abrir bilhete com ${slipCount} seleções`}><Layers3 size={22} />{slipCount > 0 && <b className="bet-slip-count">{slipCount}</b>}<span>Boletim</span></button>
      ...
      <AnimatePresence>
        ...
        {mobileSlip && <motion.div className="mobile-overlay align-bottom" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setMobileSlip(false)}><motion.div className={`bottom-sheet mobile-betslip-sheet ${slipCount <= 2 ? "compact-slip" : ""}`} role="dialog" aria-modal="true" aria-label="Bilhete de aposta" initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", damping: 28, stiffness: 320 }} onClick={(event) => event.stopPropagation()}><span className="sheet-handle" /><BetSlip mobile onClose={() => setMobileSlip(false)} /></motion.div></motion.div>}
      </AnimatePresence>
    </div>
  );
}
