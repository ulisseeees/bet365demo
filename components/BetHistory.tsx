      ...
      <div className="bet-history-list">
        {visible.map((bet) => (
          <motion.article layout className={`bet-history-card bet-${bet.status}`} key={bet.id}>
            ...
            {expanded === bet.id && <motion.div className="bet-selections-detail" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }}>{bet.selections.map((selection) => <div key={selection.id}><span><strong>{selection.selectionLabel}</strong><small>{selection.matchLabel} • {selection.marketName}</small></span><b>{selection.currentOdd && selection.currentOdd !== selection.odd ? <><del>{formatOdd(selection.odd)}</del> {formatOdd(selection.currentOdd)}</> : formatOdd(selection.odd)}</b></div>)}{bet.status === "pending" && <div className="cashout-row"><span><WalletCards size={18} /><span><strong>Cash out</strong><small>A oferta atual é confirmada automaticamente ao solicitar.</small></span></span><button className="btn btn-primary" disabled={loadingCashout === bet.id} onClick={async () => { setLoadingCashout(bet.id); await cashOut(bet.id); setLoadingCashout(null); }}>{loadingCashout === bet.id ? <LoaderCircle className="spin" size={15} /> : <WalletCards size={15} />} Fazer cash out</button></div>}</motion.div>}
          </motion.article>
        ))}
