...
export function HomePromotions({ matches, onOpenMatch, onOpenRewards }: { matches: Match[]; onOpenMatch: (match: Match) => void; onOpenRewards: () => void }) {
  ...
  return (
    <section className="promo-showcase" aria-label="Ofertas em destaque">
      <motion.article className="promo-banner super-odd-promo" whileHover={{ y: -3 }}>
        <span className="promo-icon"><Sparkles size={24} /></span>
        <div><small>{boosted ? "SUPER ODD ATIVA" : "MÚLTIPLA TURBO"}</small><h3>{boosted ? boosted.option.label : "Retorno turbinado"}</h3><p>{boosted ? `${boosted.match.home} × ${boosted.match.away}` : "Monte sua múltipla e desbloqueie boosts progressivos."}</p></div>
        ...
        <button onClick={() => boosted ? onOpenMatch(boosted.match) : onOpenRewards()}>{boosted ? "Ver mercado" : "Ver vantagens"} <ArrowRight size={15} /></button>
      </motion.article>

      <motion.article className="promo-banner vip-promo" whileHover={{ y: -3 }}>
        <span className="promo-icon"><Crown size={24} /></span>
        <div><small>ARENA CLUB</small><h3>Seu nível: {level}</h3><p>Cashback, Free Bets e benefícios que crescem com você.</p></div>
        <strong className="promo-highlight">{brl(cashback)}<small>cashback</small></strong>
        <button onClick={onOpenRewards}>Abrir clube <ArrowRight size={15} /></button>
      </motion.article>

      <motion.article className="promo-banner mission-promo" whileHover={{ y: -3 }}>
        <span className="promo-icon">{mission?.completed ? <Trophy size={24} /> : <Target size={24} />}</span>
        <div><small>MISSÃO DA SEMANA</small><h3>{mission?.title ?? "Rota da Copa"}</h3><p>{mission?.completed ? "Missão concluída. A Free Bet já entrou na conta." : mission?.description ?? "Aposte na Copa e conquiste uma Free Bet."}</p></div>
        ...
        <button onClick={onOpenRewards}>{mission?.completed ? <Gift size={14} /> : null} Ver missão <ArrowRight size={15} /></button>
      </motion.article>
    </section>
  );
}
